...
const NOTE_COLORS = [
  ...
  { label: 'Blue',     value: 'bg-orange-50 dark:bg-blue-950',          border: 'border-blue-200 dark:border-blue-800' },
      ...
      <div className={`flex flex-col ${currentColor.value} border ${currentColor.border} rounded-2xl shadow-2xl overflow-hidden transition-all ${isFullscreen ? 'w-full h-full rounded-none' : 'w-full max-w-3xl max-h-[90vh]'}`}>
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-200 dark:border-slate-700">
          ...
          <div className="flex items-center gap-1">
                ...
                <div className="absolute right-0 top-9 z-50 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden w-44">
                  ...
                  {folders.map(f => (
                    <button key={f.id} onClick={() => setFolder(f.id)}
                      className={`w-full text-left px-3 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 text-sm text-slate-600 dark:text-slate-300 ${draft.folderId === f.id ? 'bg-orange-50 dark:bg-blue-950' : ''}`}>
                      {f.icon} {f.name}
                    </button>
                  ))}
                </div>
            ...
            <button onClick={() => toggleProp('isLocked')} title="Lock"
              className="p-1.5 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">
              {draft.isLocked ? <Lock size={16} className="text-orange-500" /> : <Unlock size={16} className="text-slate-400" />}
            </button>
            {/* Archive */}
        ...
        {showTagInput && (
          <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-200 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-800/40">
            ...
            />
            <button onClick={addTag} className="text-xs text-orange-500 font-medium">Add</button>
          </div>
        )}
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            {note.isPinned && <Pin size={11} className="text-orange-400 shrink-0" />}
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 right-2 flex items-center gap-1 z-10">
        {note.isPinned && (
          <span className="p-1 rounded-full bg-orange-100 dark:bg-blue-900/50">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 bg-transparent text-sm text-slate-800 dark:text-slate-100 outline-none focus:border-orange-500"
            />
            <button
              onClick={handleAdd}
          ...
          <div className="flex flex-wrap gap-2">
            {FOLDER_ICONS.map(i => (
              <button
                key={i}
                onClick={() => setIcon(i)}
                className={`w-8 h-8 rounded-lg text-base transition-all ${icon === i ? 'bg-orange-100 dark:bg-blue-900 scale-110' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}
              >{i}</button>
            ))}
          </div>
          {/* Color picker */}
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-950 overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 shrink-0">
            ...
            <div className="absolute right-0 top-9 z-30 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl w-36 overflow-hidden">
              {(['updatedAt','createdAt','title','wordCount'] as SortBy[]).map(s => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 ${sortBy === s ? 'text-orange-500 font-medium' : 'text-slate-600 dark:text-slate-300'}`}
                >
        ...
        <div className="flex items-center rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          {(['grid','list','compact'] as ViewMode[]).map(m => (
            <button
              ...
              className={`p-1.5 transition-colors ${viewMode === m ? 'bg-orange-500 text-white' : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
        ...
        <button
          onClick={createNote}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-orange-500 hover:bg-gradient-to-br from-orange-500 to-amber-400 text-white text-sm font-medium transition-colors shadow-sm"
        >
          <Plus size={15} />
      ...
      <div className="flex items-center gap-2 px-4 py-2 overflow-x-auto scrollbar-none border-b border-slate-100 dark:border-slate-800 shrink-0">
        {([
          ...
          <button
            ...
            className={`shrink-0 px-3 py-1 rounded-full text-xs font-medium transition-all ${filterMode === f.id ? 'bg-orange-500 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
          >{f.label}</button>
        ))}
        ...
        <button
          ...
          className="shrink-0 flex items-center gap-1 px-3 py-1 rounded-full text-xs text-slate-400 hover:text-orange-500 hover:bg-orange-50 dark:hover:bg-blue-950/40 border border-dashed border-slate-300 dark:border-slate-600 transition-all"
        >
        ...
        <div className="flex items-center gap-2 px-4 py-2 overflow-x-auto scrollbar-none border-b border-slate-100 dark:border-slate-800 shrink-0">
          ...
          {allTags.map(tag => (
            <button
              ...
              className={`shrink-0 px-2 py-0.5 rounded-full text-xs transition-all ${selectedTags.includes(tag) ? 'bg-orange-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
            >#{tag}</button>
          ))}
      ...
      <div className="flex items-center gap-4 px-4 py-1.5 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 text-xs text-slate-400 shrink-0">
        ...
        {searchQuery && <span className="text-orange-500">Searching: "{searchQuery}"</span>}
        {selectedFolder && <span className="text-orange-500">📁 {folders.find(f => f.id === selectedFolder)?.name}</span>}
        {selectedTags.length > 0 && <span className="text-orange-500">Tags: {selectedTags.map(t => `#${t}`).join(', ')}</span>}
      </div>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-slate-400">
            ...
            {!searchQuery && filterMode === 'all' && (
              <button
                onClick={createNote}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-500 hover:bg-gradient-to-br from-orange-500 to-amber-400 text-white text-sm font-medium transition-colors"
              >
