...
import { format, subDays, isToday, parseISO, differenceInDays, startOfWeek, addDays } from 'date-fns';
...
const HabitView: React.FC<HabitViewProps> = ({
  ...
  onMenuClick, onOpenStats, onStartFocus,
}) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-[#0f0f1a] overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 pt-12 pb-4 md:pt-6 bg-white dark:bg-slate-950 border-b border-slate-100 dark:border-white/5">
        ...
        <button onClick={onOpenStats} className="w-9 h-9 flex items-center justify-center rounded-xl text-slate-500 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-500 transition-colors" title="Stats">
          <BarChart2 size={20} />
        </button>
