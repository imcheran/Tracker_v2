...
const TaskItem: React.FC<TaskItemProps> = ({ task, onToggle, onSelect, selected }) => {
  ...
  // isPast polyfill: check if due date is before now and not today
  ...
  return (
    <div 
      ...
      className={`
        ...
        ${selected ? 'bg-indigo-50 border-blue-100' : 'bg-white hover:bg-slate-50'}
        ...
        <button 
          ...
          className={`
            ...
            ${task.isCompleted 
                ...
                : `border-slate-300 hover:border-indigo-500 ${priorityColor[task.priority].replace('text-', 'border-')}`
            }
          `}
        >
