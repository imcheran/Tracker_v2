                    ...
                    <div className="p-6">
                        ...
                        <div className="space-y-4 mb-8">
                            ...
                            <label className={`flex items-center gap-3 cursor-pointer group ${(habit.streakFreezes || 0) > 0 || habit.freezeDates?.includes(format(selectedDate, 'yyyy-MM-dd')) ? '' : 'opacity-50 pointer-events-none'}`}>
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${logStatus === 'skipped' ? 'border-blue-500' : 'border-slate-600'}`}>
                                    {logStatus === 'skipped' && <div className="w-3 h-3 bg-blue-500 rounded-full"/>}
                                </div>
