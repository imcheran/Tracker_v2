...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const getTaskStyle = (task: Task) => {
      ...
      switch (task.priority) {
          ...
          default: return { bg: 'bg-indigo-50 dark:bg-orange-900/20', border: 'border-l-4 border-indigo-500', text: 'text-blue-900 dark:text-blue-100' };
      }
  };
                                  ...
                                  return (
                                      <div key={day.toString()} className="bg-white dark:bg-slate-900 p-1 min-h-[30px] flex flex-col gap-1">
                                          {allDayTasks.map(task => (
                                              <div key={task.id} onClick={() => onSelectTask(task.id)} className={`text-[9px] px-1.5 py-0.5 rounded truncate font-medium cursor-pointer ${task.isEvent ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300' : 'bg-indigo-100 text-blue-700 dark:bg-orange-900/30 dark:text-orange-300'} ${task.isCompleted ? 'opacity-50 line-through' : ''}`}>{task.title}</div>
                                          ))}
                                      </div>
                                  );
                              })}
