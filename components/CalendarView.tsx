...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const getTaskStyle = (task: Task) => {
      ...
      switch (task.priority) {
          ...
          default: return { bg: 'bg-indigo-50 dark:bg-indigo-900/20', border: 'border-l-4 border-indigo-500', text: 'text-blue-900 dark:text-blue-100' };
      }
  };
                          ...
                          >
                              ...
                              <div className="space-y-1 overflow-hidden flex-1 px-0.5">
                                  {dayTasks.slice(0, 3).map(t => (
                                      <div key={t.id} className={`text-[8px] px-1.5 py-0.5 rounded-md truncate leading-tight ${t.isEvent ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 border-l-2 border-indigo-500' : 'text-slate-700 dark:text-slate-300 bg-indigo-50/50 dark:bg-indigo-900/30 border-l-2 border-indigo-400 dark:border-indigo-500'} ${t.isCompleted ? 'opacity-50 line-through' : ''}`}>{t.title}</div>
                                  ))}
                                  ...
                                  return (
                                      <div key={day.toString()} className="bg-white dark:bg-slate-900 p-1 min-h-[30px] flex flex-col gap-1">
                                          {allDayTasks.map(task => (
                                              <div key={task.id} onClick={() => onSelectTask(task.id)} className={`text-[9px] px-1.5 py-0.5 rounded truncate font-medium cursor-pointer ${task.isEvent ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300' : 'bg-indigo-100 text-blue-700 dark:bg-indigo-900/30 dark:text-indigo-300'} ${task.isCompleted ? 'opacity-50 line-through' : ''}`}>{task.title}</div>
                                          ))}
                                      </div>
                                  );
                              })}
